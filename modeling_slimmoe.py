...
# Make flash_attn optional for CPU inference
try:
...
# Compatibility helper for cache API changes
def _get_cache_length(cache, default_length=0, layer_idx=None):
    """Get cache length with compatibility for different transformers versions."""
    if hasattr(cache, 'get_usable_length'):
        # Old API
        if layer_idx is not None:
            return cache.get_usable_length(default_length, layer_idx)
        else:
            return cache.get_usable_length(default_length)
...
class PhiMoEModel(PhiMoEPreTrainedModel):
    ...
    def forward(
        ...
        if use_cache:
            ...
            past_key_values_length = _get_cache_length(past_key_values, seq_length)
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if past_key_values and self.config.rope_scaling and input_ids.shape[1] >= self.config.original_max_position_embeddings + 1:
            # Handle both old and new Cache API
            if isinstance(past_key_values, Cache):
                past_length = getattr(past_key_values, 'seen_tokens', past_key_values.get_seq_length())
            else:
        ...
        if past_key_values is not None:
            if isinstance(past_key_values, Cache):
                ...
                max_cache_length = past_key_values.get_max_cache_shape() if hasattr(past_key_values, 'get_max_cache_shape') else None
            else:
